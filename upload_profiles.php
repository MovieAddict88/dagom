...
if (isset($_FILES['profiles_ovpn']) && isset($_POST['profile_type'])) {
    ...
    $promo_ids = (isset($_POST['promo_ids']) && is_array($_POST['promo_ids'])) ? $_POST['promo_ids'] : [];
    ...
    if (!empty($promo_ids)) {
        foreach ($promo_ids as $promo_id) {
            ...
            $stmt->execute();
            if ($stmt->rowCount() == 0) {
                $error_message = 'Invalid promo selected: ' . $promo_id;
                break;
            }
        }
    }
                    ...
                    if ($stmt->execute()) {
                        ...
                        if (!empty($promo_ids)) {
                            ...
                            foreach ($promo_ids as $pid) {
                                $stmt_promo->execute([':profile_id' => $profile_id, ':promo_id' => $pid]);
                            }
                        }
                        $upload_count++;
                    }
...
<div class="card mt-4">
    ...
    <div class="card-body">
        ...
        <form action="upload_profiles.php" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>Promos (App will expect these too)</label>
                <div style="max-height: 200px; overflow-y: auto; border: 1px solid #ccc; padding: 10px; border-radius: 4px;">
                    ...
                    foreach ($promos as $promo) {
                        ...
                        echo '<label><input type="checkbox" name="promo_ids[]" value="' . $promo['id'] . '"> ' . htmlspecialchars($promo['promo_name']) . '</label>';
                        echo '</div>';
                    }
                    ?>
                </div>
